...
class TestLambdaWizard(BaseTestCase):
    ...
    def test_list_lambda_functions_success(self):
        self.lambda_client.list_functions.return_value = {
            'Functions': [
                {'FunctionName': 'func1', 'LastModified': '2025-05-20T10:00:00'}
            ]
        }
        self.lambda_client.list_aliases.return_value = {'Aliases': [{'AliasName': 'alias1'}]}
        self.lambda_client.list_versions_by_function.return_value = {'Versions': [{'Version': '1'}, {'Version': '$LATEST'}]}
    ...
    def test_list_lambda_functions_with_exception(self):
        ...
        self.lambda_client.list_aliases.side_effect = Exception("API error")
    ...
    def test_delete_lambda_function_success(self):
        ...
        with patch('wizards.lambda_wizard.log_action') as mock_log_action:
            result = lambda_wizard.delete_lambda_function('func1', self.lambda_client)

        self.lambda_client.delete_alias.assert_called_once_with(FunctionName='func1', Name='alias1')
        self.lambda_client.delete_function.assert_has_calls([
    ...
    def test_delete_lambda_function_failure(self):
        self.lambda_client.list_aliases.return_value = {'Aliases': []}
        self.lambda_client.list_versions_by_function.return_value = {'Versions': []}
        self.lambda_client.delete_function.side_effect = Exception("Delete error")

        with patch('wizards.lambda_wizard.log_action') as mock_log_action:
            result = lambda_wizard.delete_lambda_function('func_fail', self.lambda_client)
    ...
    def test_delete_selected_lambda_functions_exit_immediately(self):
        with patch('wizards.lambda_wizard.input') as mock_input, \
            ...
            self.lambda_client.list_functions.return_value = {'Functions': [{'FunctionName': 'func1', 'LastModified': 'now'}]}
    ...
    def test_delete_selected_lambda_functions_all_yes(self):
        with patch('wizards.lambda_wizard.input') as mock_input, \
            ...
            self.lambda_client.list_aliases.return_value = {'Aliases': []}
            self.lambda_client.list_versions_by_function.return_value = {'Versions': []}
            self.lambda_client.delete_function.return_value = None

            lambda_wizard.delete_selected_lambda_functions()

            self.assertEqual(self.lambda_client.delete_function.call_count, 4)
