#file name: test_s3_wizard_integration.py
import subprocess
import boto3
import re
import time
...
def test_s3_wizard_deletes_known_bucket():
    ...
    # Step 1: Run the wizard with menu option 2 to get the bucket list
    ...
    # Step 2: Extract the bucket's number from the menu using regex
    ...
    # Step 3: Run the wizard again to delete the bucket
    delete_input = f"2\n{bucket_index}\nyes\n3\n"
    result = subprocess.run(
        ["python", "s3_wizard.py"],
    ...
    # Step 4: Assert deletion message and that the bucket is now gone
