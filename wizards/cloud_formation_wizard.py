...
def delete_stack_worker(stack_name, success_list, fail_list):
            ...
            try:
                ...
                try:
                    ...
                    if status == 'DELETE_COMPLETE':
                        ...
                        with lock:
                            fail_list.append((stack_name, "Stack deletion failed (status: DELETE_FAILED)"))
                        log_action("Cloud Formation", stack_name, False, mode="deletion")

                    else:
                        raise Exception(f"Stack still exists with unexpected status: {status}")
                    ...
                    if "does not exist" in str(e):
                        ...
                        with lock:
                            fail_list.append((stack_name, f"ClientError: {str(e)}"))
                        log_action("Cloud Formation", stack_name, False, mode="deletion")
...
def delete_selected_stacks():
    ...
    if not selected_stacks:
        print("No valid stacks were selected for deletion. Returning to main menu.")
        return None, None, None
    ...
    thread = threading.Thread(
        target=_background_stack_deletion,
        args=(selected_stacks, success_list, fail_list)
    )
    thread.daemon = True
    thread.start()

    print("Deletion has been initiated in the background. You can continue working.")
    return thread, success_list, fail_list
